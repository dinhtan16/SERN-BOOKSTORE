...
const Shop = () => {
  ...
  const bookDataLimit = useSelector(
        ...
        <div className="filter-sidebar">
          ...
          <CAccordion alwaysOpen activeItemKey={1}>
                  ...
                  {priceFilerData?.map((item) => {
                    return (
                      <div
                        ...
                        onClick={() => handleFilterPrice(item.code)}
                      >
                        {item.value}
                      </div>
                    );
            ...
            <CAccordionItem itemKey={2}>
              <CAccordionHeader>  <h6>Increase/Decrease</h6></CAccordionHeader>
                  ...
                  {orderPrice?.map((item) => {
                    return (
                      <div
                        key={item.id}
                        className="filter-item"
                        onClick={() => handleFilterOrder(item.sort)}
                      >
                        {item.value}
                      </div>
                    );
                  ...
                  {categoryData?.map((item, i) => {
                    return (
                      <div
                        key={item.code}
                        className="filter-item"
                        onClick={() => handleFilterCategory(item.code)}
                      >
                        {item.value}
                      </div>
                    );
